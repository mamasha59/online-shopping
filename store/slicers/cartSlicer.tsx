...
const initialState:iInitialState = { // первоначальное состояние карзины это пустой массив
...
const cartAddProductSlice = createSlice({
    ...
    reducers: {
      addProductIntoCart: (state,action:PayloadAction<iProduct>) => {
        const itemExist = state.items.find(obj=>obj.product.id === action.payload.id);
        if(itemExist){
          itemExist.quantity ++
        }else{
      ...
      addOneMore:(state,action:PayloadAction<{id:number}>) => {
        const current = state.items.find((obj) => obj.product.id === action.payload.id);
...
export const { addProductIntoCart, removeProductFromCart, addOneMore, deleteOne } = cartAddProductSlice.actions; // импортируем экшены
