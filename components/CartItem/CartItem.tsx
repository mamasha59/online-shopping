  ...
  return (
      <div className="w-full flex my-7 justify-between bg-[#8080800b] rounded-sm border-b border-[#80808020] md:flex-col midl:py-2">
        <div className="flex grow-0 shrink flex-[80%] items-center midl:flex-col">
          <Image width={200} height={250} src={item.product.img} alt={item.product.altDescription} className="md:max-w-[150px] md:max-h-[150px]"/>
          <div className="ml-3 p-3 flex flex-col">
            <div className="max-w-2xl">
              <h3 className="text-[#0c0909] font-medium text-2xl md:text-lg">{item.product.title}</h3>
        ...
        <p className="p-3 text-xl font-semibold flex justify-end grow-0 shrink-0 flex-[20%]">&#8381; {item.product.price}</p>
      </div>
  );
};
